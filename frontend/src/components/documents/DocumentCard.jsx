      ...
      <div className='mt-5 pt-4 border-t border-slate-100'>
        <div className='flex items-center gap-1.5 text-xs text-slate-500'>
          <Clock
            ...
            strokeWidth={2}
          />
          <span>Uploaded on {moment(document.createdAt).format('MMM DD, YYYY')}</span>
        </div>
      </div>

      {/* Hover Indicator */}
      <div className='absolute inset-0 bg-linear-to-br from-emerald-500 to-teal-500 opacity-0 group-hover:opacity-10 transition-opacity duration-200 rounded-xl pointer-events-none' />
    </div>
  );
};
